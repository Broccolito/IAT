...
pointset = data %>%
  filter(condition == "diseased") %>%
  filter(file_name == "1272902.txt")

find_theta = function(pointset,
                      left_bottom_index = 7,
                      left_up_index = 4,
  ...
  d$index = 1:dim(d)[1]
  d$anno = NA
  d$anno[d$index %in% (marker_index_start:marker_index_end)] = "marker"
  d$anno[d$index %in% (superior_index_start:superior_index_end)] = "superior"
  d$anno[d$index %in% (insertion_index_start:insertion_index_end)] = "insertion"
  ...
  for(rotation_angle in rotation_angles){
    ...
    rotational_loss = mean(rotational_loss, na.rm = TRUE)
